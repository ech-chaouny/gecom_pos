    ...
    <>
      {!loading ? (
        ...
        <>
          {articles.length <= 0 ? (
            <div className="d-flex flex-row justify-content-center align-items-center pt-5">
              <h4 className="text-gray-900 fw-bold">Produits introuvables !</h4>
            </div>
          ) : (
            <div className="d-flex flex-wrap product-list-block__product-block">
              {articles &&
                articles.map((article, index) => (
                  ...
                  >
                    <div
                      className={`position-relative ${activeCard(
                        article.id
                      )} card`}
                    >
                      <img
                        className="card-img-top"
                        src={`http://127.0.0.1:8000/storage/uploads/Articles/${article.photo}`}
                        alt="article"
                      />
